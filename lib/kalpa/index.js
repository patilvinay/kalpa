...
//* ************************************************************************************** */
const processKalpaRoot = (obj, __data) => {
...
const renderSourceFile = (file) => {
  ...
  rootNode.__data =(root.__data===undefined? {}:root.__data)
  if(path.isAbsolute(file)){
    rootNode.__data.cwd = path.dirname(file);
  } else {
    rootNode.__data.cwd=path.resolve(".")
  }
  let bwd=path.resolve(".");
  let relPath= path.relative(bwd,file)
