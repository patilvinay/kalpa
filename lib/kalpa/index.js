...
const renderChapter = (chapter) => {
    ...
    let modified = false;
    if (checkIfStrModified(str1)) {
        modified = true
        str1 = str1.slice(0, str1.length - 5);
    }
    ...
    if (true === modified) {
        vars = vars = decryptRenderedString(str);
    } else {
        vars = str.replace(/&quot;/g, '');
    }
...
const renderVars = (rootData) => {
  ...
  const str = nunjucks.renderString(str1, {
    ...
    let vars;
    if (true === modified) {
        vars = decryptRenderedString(str);
    } else {
        vars = str.replace(/&quot;/g, '');
    }
    vars = vars.replace(/{a{/g, "{{<");
...
const decryptRenderedString = (str) => {
    let tempStr;
    let testStr = str, startIndex;
    let quotIndex, tempIndex;
    let originalEndIndex, colonIndex;
    // console.log(str)
    originalEndIndex = 0;
    for (;;) {
        quotIndex = testStr.search('&quot;');
        if (quotIndex === -1) {
            break;
        }
        tempStr = testStr.slice(0, quotIndex);
        ...
        tempIndex = tempStr.search('\"')
        console.log('abcd colonIndex:', colonIndex, startIndex, tempIndex, testStr)
        if (tempIndex === -1) {
            let braceCount = 0;
            for (; startIndex < testStr.length; startIndex++) {
                if (startIndex > quotIndex) {
                    if (braceCount <= 0) {
                        originalEndIndex = startIndex
                        break;
                    }
                }
                if ((testStr.charAt(startIndex) === '{') || (testStr.charAt(startIndex) === '[') || (testStr.charAt(startIndex) === '(')) {
                    braceCount++;
                } else if ((testStr.charAt(startIndex) === '}') || (testStr.charAt(startIndex) === ']') || (testStr.charAt(startIndex) === ')')) {
                    braceCount--;
                }
            }
            for (;;) {
                tempIndex = testStr.search('&quot;');
                if (tempIndex === -1) {
                    break;
                }
                if (tempIndex <= originalEndIndex) {
            ...
            tempStr = testStr.slice(quotIndex, testStr.length);
            tempIndex = tempStr.search('\"');
            originalEndIndex = (quotIndex + tempIndex);
            console.log(quotIndex, tempIndex)
            for (;;) {
                tempIndex = testStr.search('&quot;');
                if (tempIndex === -1) {
                    break;
...
const checkIfStrModified = (str) => {
    const tempStr = str.slice(str.length - 5, str.length);
    if (tempStr.includes('vivek')) {
        return true;
    } else {
        return false;
    }
...
const prepareDumpObjectForRendering = (str) => { 
  let testStr = str
  let tempStr, dumpIndex, endIndex, dumpObject;
  let temp1Str, startIndex;
  let originalStartIndex, originalEndIndex;
  let retStr = str;
  let modified = false;

  originalEndIndex = originalStartIndex = 0;
  for (;;) {
    dumpIndex = testStr.search('dump');
    if (dumpIndex === -1) {
        break;
    }
    saveIndex = -1;
    tempStr = testStr.slice(0, dumpIndex);
    for (i = 0; ;i++) {
        startIndex = tempStr.search('\"{{-');
        if (startIndex !== -1) {
            originalStartIndex += (startIndex + (i * 3));
            saveIndex = startIndex;
            temp1Str = tempStr;
            tempStr = tempStr.slice(startIndex + 3, tempStr.length)
        } else {
            if (saveIndex >= 0) {
                if (temp1Str.charAt(saveIndex) !== '\"') {
                    saveIndex = -1;
                } else {
                    break;
                }
    ...
    tempStr = testStr.slice(dumpIndex, testStr.length)
    endIndex = tempStr.search('-}}');
    if (endIndex !== -1) {
        if ((saveIndex !== -1) && (tempStr.charAt(endIndex + 3) === '\"')) {
            originalEndIndex += (dumpIndex + endIndex + 3);
            retStr = retStr.replaceAt(originalStartIndex, " ");
            retStr = retStr.replaceAt(originalEndIndex, " ");
            modified = true;
            testStr = tempStr.slice(endIndex + 3, tempStr.length)
            originalStartIndex = originalEndIndex
        } else {
            testStr = tempStr.slice(endIndex + 3, tempStr.length)
            originalStartIndex += (dumpIndex + endIndex + 3);
            originalEndIndex = originalStartIndex
        }
    } else {
        testStr = tempStr.slice(4, tempStr.length)
        originalStartIndex = originalEndIndex = dumpIndex + 4;
    }
  }
  if (modified === true) {
    retStr = retStr + 'vivek'
