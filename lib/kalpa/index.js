...
const renderChapter = (chapter) => {
    ...
    const pattern = /"?[\w_ ]*\{\{[\w|\.<> ]*\}\}[\w ]*"?/g
    const paramsPattern = /" *?\{{2}[\w| _]*\}{2} *?"/g;
    let matchStr, str, index;
  
    while(1) {
      matchStr = pattern.exec(str1);
      if (matchStr === null) {
          break;
      }
      str = nunjucks.renderString(matchStr[0], {
        vars: Var,
        imported: _import,
        ctx,
        env: process.env,
      ...
      if (matchStr[0].match(paramsPattern) === null) {
          str = str.replace(/&quot;/g, '')
      } else {
          str = str.replace('"', '')
      ...
      str1 = str1.replace(matchStr[0], str)
    }
